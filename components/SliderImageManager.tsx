...
import { Slider } from '../services/sliders'; 
import { 
  useGetImages,
  useAddImagesToSlider,
  useRemoveImagesFromSlider,
  useDeleteImage,
  Image
...
export default function SliderImageManager({ slider, onClose }: SliderImageManagerProps) {
  ...
  const { mutate: addImagesById, isPending: isAddingById } = useAddImagesToSlider();
  const { mutate: removeImagesById, isPending: isRemovingById } = useRemoveImagesFromSlider();
  ...
  // Initialiser la sélection depuis le slider prop
  useEffect(() => {
    // 💡 Leçon : S'assurer que slider.images existe avant de mapper
    const ids = new Set<string>(slider.images?.map((i) => i.id) || []);
    setSelectedIds(new Set(ids));
    setInitialIds(new Set(ids));
  }, [slider, allImages]); // Ajout de allImages comme dépendance pour recalculer si la liste change
  ...
  const handleSaveSelection = () => {
    ...
    if (toAdd.length > 0) {
      addImagesById({ sliderId: slider.id, imagesIds: toAdd }, {
        onSuccess: () => {
          toast.success(`${toAdd.length} image(s) ajoutée(s) au slider.`);
        },
        onError: () => {
          errors = true;
          toast.error("Erreur lors de l'ajout d'images au slider.");
        }
      });
    }
    ...
    if (toRemove.length > 0) {
      removeImagesById({ imagesIds: toRemove }, {
        onSuccess: () => {
          toast.success(`${toRemove.length} image(s) retirée(s) du slider.`);
        },
        onError: () => {
          errors = true;
          toast.error("Erreur lors de la suppression d'images du slider.");
        }
      });
    }
  ...
  return (
    <div className="space-y-8 p-4">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mb-4">Sélectionnez les images existantes à (dé)lier de ce slider. N'oubliez pas d'enregistrer.</p>
        ...
        {isLoadingAllImages ? (
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 max-h-96 overflow-y-auto p-2 border rounded-lg">
            {allImages?.map((image: Image) => {
              ...
              // L'image est-elle liée à un *autre* slider ?
              const isUsedElsewhere = image.sliderId !== null && image.sliderId !== slider.id;
              ...
              return (
                ...
                >
                  ...
                  `}>
                    ...
                    {isUsedElsewhere && (
                        <span className="text-white text-xs text-center font-bold">Utilisée par un autre slider</span>
                    )}
                  </div>
                </div>
              );
